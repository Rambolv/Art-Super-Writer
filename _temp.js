...
function loadDB(){
  ...
  // 如果 localStorage 为空，从磁盘加载项目索引（跨浏览器共享）
  if(!Object.keys(DB.projects).length) loadProjectIndexFromDisk();
...
// 从磁盘加载项目索引（跨浏览器共享）
async function loadProjectIndexFromDisk(){
  try {
    ...
    if(data.projects && data.projects.length){
      for(const name of data.projects){
        if(!DB.projects[name]){
          DB.projects[name] = {name, folders: createDefaultFolders(), imported: true, analyzed: false, createdAt: Date.now()};
        }
      }
      if(typeof renderers?.project === 'function') renderers.project();
    }
  } catch(e){}
}
...
async function saveDBToDisk(){
  try {
    const indexData = Object.keys(DB.projects).map(name => ({
      name, analyzed: !!DB.projects[name].analyzed,
      imported: !!DB.projects[name].imported,
      chCount: (DB.projects[name].chapters||[]).length
    }));
    await fetch('http://127.0.0.1:8899/api/project/save-file', {
      method:'POST',headers:{'Content-Type':'application/json'},
      body:JSON.stringify({project:'__asr_writer_meta__', folder:'', filename:'project_index.json', content:JSON.stringify(indexData)})
    });
  } catch(e){}
}
...
async function loadChaptersFromDisk(projectName){
  ...
  // 只加载 chapter_NNN.md 格式的章节文件（不加载 part_ 和 full_text）
...
async function analyzeProject(name){
  ...
  try {
      ...
      for(let pi=0; pi<textParts.length; pi++){
        ...
        if(estimateTokens(partFull) <= optimalMax){
          ...
          for(let si=0; si<subParts; si++){
            ...
            const subReply = await callLLM(partConv, {max_tokens:100, temperature:0.1});
        ...
        const partMarkConv = [
          {role:'system', content:'你是一个小说章节分析助手。用户已经给你发送了' + partLabel + '的全文。现在请你根据记忆，一条一条发送章节信息。'},
          {role:'user', content:'这是' + partLabel + '的全文，约'+partFull.length+'字。开头：'+partFull.substring(0,200)+'\n\n结尾：'+partFull.substring(partFull.length-200)+'\n\n请逐条发送章节信息。对每章提供：章节编号、章节标题、该章开头一段独特原文文字（20-40字）、该章结尾一段独特原文文字（20-40字）。\n\n格式：\n第1章 | 完整标题 | 【章首】开头20-40字原文 | 【章尾】结尾20-40字原文\n\n注意：【章首】和【章尾】后面的文字必须是原文中的原句，不要改写。\n默认情况下，文章开头就是第一章。\n\n请一条条发，我收到后会划分并保存该章，然后回复"已存档，划分好章节，请发下一条"。全部发完后请回复"所有章节已发完"。'}
        ];
        ...
        while(!partDone && partTries < 50){
          partTries++;
          const preply = await callLLM(partMarkConv, {max_tokens:1024, temperature:0.1});
          ...
          for(const line of preply.split('\n')){
            ...
            if(content.length>50){
              ...
              partSearchFrom=ep;
              partMarkConv.push({role:'assistant',content:'已存档，划分好章节，请发下一条'});
              break;
            }
          }
        }
    ...
    for(let i=0; i<batches.length; i++){
      ...
      logAnalysis('第2步 第'+(i+1)+'批', preview+'...', reply.substring(0,100));
    ...
    // 新建精简对话（不带全文原文，只带文本概览和提示，避免撑爆上下文）
    const step3Conv = [
      {role:'system', content:'你是一个小说章节分析助手。用户已经给你发送了整部小说的全文。现在请你根据记忆，一条一条发送章节信息。'},
      {role:'user', content:'小说的全文我已经发送给你了。全文约'+fullText.length+'字。开头内容是：'+fullText.substring(0,200)+'\n\n结尾内容是：'+fullText.substring(fullText.length-200)+'\n\n请逐条发送章节信息。对每章提供：章节编号、章节标题、该章开头一段独特原文文字（20-40字）、该章结尾一段独特原文文字（20-40字）。\n\n格式如下：\n第1章 | 完整标题 | 【章首】该章开头20-40字原文 | 【章尾】该章结尾20-40字原文\n\n注意：【章首】和【章尾】后面的文字必须是原文中的原句，不要改写。\n默认情况下，文章开头就是第一章，除非作者明确写明是序章，遇到序章的情况直接把文章开头作为第一章，后面的第一章作为第二章，以此类推。\n\n请一条条发，我收到后会划分并保存该章，然后回复"已存档，划分好章节，请发下一条"。你收到确认后再发下一条。全部发完后请回复"所有章节已发完"。'}
    ];
    const chMarks = [];
    // 归一化函数
    function _norm1(t){ return t.replace(/[\uff01-\uff5e]/g,c=>String.fromCharCode(c.charCodeAt(0)-0xfee0)).replace(/\u3000/g,' '); }
    ...
    while(!step3Done && step3Tries < 100){
      step3Tries++;
      const reply = await callLLM(step3Conv, {max_tokens:1024, temperature:0.1});
      ...
      for(const line of reply.split('\n')){
        ...
        if(content.length > 50){
          ...
          searchFrom = endPos;
          matchedAny = true;
          step3Conv.push({role:'assistant',content:'已存档，划分好章节，请发下一条'});
          break;
        }
      }
      if(!matchedAny){
        step3Conv.push({role:'assistant', content:'我无法根据你发送的信息划分章节，请发送更多章节信息以供匹配，或者请自行划分清晰的章节信息。如果不需要划分章节了，请发送所有章节已发完'});
      }
    }
