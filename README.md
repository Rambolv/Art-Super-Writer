...
打开 DeepSeek 聊天框写小说，最大的问题是：**文章写长了，AI 忘记了前面小说的内容，漏了伏笔或者人物。**

超逸写手做的事情，就是把这些「自己来」变成「自动」。

| 问题 | 传统人工手写 | 直接用 AI 聊天框写 | 用超逸写手 + API |
|------|------------|------------------|----------------|
| 章节越长，前情矛盾 | 😩 靠脑子记，越写越容易忘 | 😩 每次手动复制粘贴前文，仍然忘 | ✅ 自动把你全部小说发给 AI 读完，再动笔 |
| 角色前后不一致 | 😩 手动做角色卡，常更新不及时 | 😩 角色写着写着性格变了、死了的又活了 | ✅ 角色档案自动管理。死了就是死了 |
| 写得好不好谁来评 | 😩 自己读自己改，容易有盲区 | 😩 写完了不知道好不好，全靠自己感觉 | ✅ 三个 AI 审稿官从 13 个维度打分 + 给修改建议 |
| 故事结构失控 | 😩 全靠大纲，写着写着走偏 | 😩 伏笔忘了收、支线丢了 | ✅ 世界树自动追踪：主线到哪了、支线进度、伏笔收没收 |
| Token 费用 | N/A | 😩 每次聊天都重新计费，Tokens 越用越多 | ✅ 双模式：⚡Slot 模式全文预热一次，后续只算新增内容，一次写作任务中可省 Tokens（需要大模型支持）；🔵Stateless 模式兼容性更好 |
| 写作风格稳定 | 😩 靠自己维持 | 😩 每次 AI 写的风格都不一样 | ✅ SKILL 系统：你定规则，AI 遵守。AI 还从你的小说里自动学 |
| 上下文越长，AI 智商 | N/A | 😩 聊天记录太长了大模型智商变低 | ✅ 所有数据存硬盘本地项目。提供模糊记忆功能，智能算法精简压缩，让大模型智商不变低同时保持任务能力 |
| 读者阅读体验感知 | 😩 靠自己和亲友反馈 | 😩 不知道读者读到每章的感受 | ✅ 节奏控制台：哪章吸引人、哪章读者可能弃书 |
| 能力扩展 | N/A | 😩 所有能力只有一个大模型提供，能力上限有限 | ✅ 本项目可选择多个大模型提供联合服务 |
...
- **独立压缩 Slot**：压缩过程使用独立 slot ID，不影响主分析会话的 KV 缓存（本地模型需要支持slot模式，云服务商一般不支持slot模式）。
