...
- 直接在这个框里修改文字（想怎么改就怎么改）
- 不满意可以点 **「🔄 重新生成」**，但上一版不保留，先复制你喜欢的部分
...
| 面板 | 这是啥 |
|------|--------|
| 📦 前置 SKILL | 你写给 AI 的写作规则。比如「对话简短，一人一次不超过三句」「用动作带说话人，不用说道」。保存勾选就生效 |
...
| 角色池 | 🔍 **分析** | **针对某个角色**做深度分析 | 想深入理解某个角色时 |
...
三页顶部都有版本选择器——每次分析生成一个新版本（v1/v2/v3），可以切换对比。
...
Sidebar → ⚙️ Settings → Add LLM → Provider: **DeepSeek** → Model: `deepseek-chat` → paste API Key (get it from [platform.deepseek.com](https://platform.deepseek.com)) → Mode: **⚡ Slot** → Save → Test.
