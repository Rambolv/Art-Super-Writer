...
function loadDB(){
  try{
    ...
    if(d){const p=JSON.parse(d);Object.assign(DB,p)}
  }catch(e){}
  // 迁移：确保所有项目都有 folders 结构
...
function saveDB(){
  localStorage.setItem('asw_db',JSON.stringify({
    projects:DB.projects,currentProject:DB.currentProject,lockedProject:DB.lockedProject,
...
function tryCreateDiskFolders(name){
  fetch('http://127.0.0.1:8899/api/project/create', {
    method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({name})
  }).catch(()=>{});
}
...
function saveProjectFilesToDisk(projectName){
  ...
  // 世界树数据
  if(p.worldTree) saveFileToDisk(projectName, 'worldTreeData', 'world_tree.json', p.worldTree);
  // 角色档案
  Object.entries(p.characters||{}).forEach(([cid, ch])=>{
    if(ch.name) saveFileToDisk(projectName, 'characterProfiles', `${ch.name}_档案.md`, `# ${ch.name}\n\n角色定位: ${ch.role||'未知'}\n\n${ch.behavioral_model||''}`);
  });
  // 节奏报告
...
function createProject(){
  ...
  DB.currentProject=name;saveDB();renderers.project();
  // 尝试创建磁盘文件夹
  ...
  for(let i=0;i<files.length;i++){
    ...
    reader.onload = (function(projName, fname){
      return function(e){
        ...
        if(DB.projects[projName]) return;
        // 仅导入原始内容，不做分析分章
        const projData = createProjectData(projName, {premise:'',genre:'未知'});
        projData.rawContent = content;
        projData.rawFile = fname;
...
function splitIntoChunks(text, maxTokens){
  ...
  const chunks = []; let current = '', ct = 0;
  for(const line of lines){
    const lt = estimateTokens(line+'\n');
    if(ct+lt>maxTokens && current.length>0){ chunks.push(current.trim()); current=''; ct=0; }
    current+=line+'\n'; ct+=lt;
  }
...
async function analyzeProject(name){
  ...
  if(!hasRaw && !hasChapters){ toast('没有章节可分析','error'); return; }
  const fullText = hasRaw ? p.rawContent : (p.chapters||[]).map(c=>c.content).join('\n\n');
  _isAnalyzing = true;
  ...
  try {
    ...
    let optimalLimit = 48000;
    if(nums){ const v=nums.map(n=>parseInt(n)*1000).sort((a,b)=>a-b); if(v.length>=2) optimalLimit=Math.floor(v[Math.floor(v.length/2)]*0.75); }
    const batchMax = Math.min(optimalLimit - 8000, 40000);
    toast(`📐 单批上限 ~${Math.round(batchMax/1000)}K`);

    // ─── 第2步：分批阅读 ───
    const conversation = [{role:'system', content:'你是一个耐心的小说读者。我会分批发送小说内容给你阅读，你读完每批后只需回复"已读"即可。等我发出分析指令时，再基于所有已读内容做分析。'}];
    const chunks = splitIntoChunks(fullText, batchMax);
    for(let i=0; i<chunks.length; i++){
      toast(`📖 第${i+1}/${chunks.length}批：阅读中...`);
      conversation.push({role:'user', content:`请阅读以下第${i+1}/${chunks.length}部分内容，读完后回复"已读"：\n\n${chunks[i]}`});
    ...
    let r = await callLLM(conversation, {max_tokens:4096, temperature:0.7});
    conversation.push({role:'assistant', content:r.substring(0,2000)});
    ...
    r = await callLLM(conversation, {max_tokens:4096, temperature:0.7});
    conversation.push({role:'assistant', content:r.substring(0,2000)});
    ...
    r = await callLLM(conversation, {max_tokens:8192, temperature:0.7});
    conversation.push({role:'assistant', content:r.substring(0,3000)});
    ...
    r = await callLLM(conversation, {max_tokens:2048, temperature:0.7});
    conversation.push({role:'assistant', content:r.substring(0,1000)});
    ...
    // ─── 第4步：综合输出 JSON ───
    toast('📋 正在整理为结构化数据...');
    conversation.push({role:'user', content:'根据以上所有分析，请输出以下JSON格式的分析结果（只输出JSON，不要其他文字）：\n\n{\n  "plot_structure": {\n    "trunk": [{"chapter":1,"event":"事件描述","importance":8}],\n    "branches": [{"name":"支线名","start_chapter":1,"status":"发展中|接近收尾","relation":7}],\n    "buds": [{"chapter":1,"foreshadow":"伏笔描述","urgency":"高|中|低"}],\n    "roots": [{"chapter":1,"category":"地点|规则|历史|其他","content":"设定描述"}]\n  },\n  "characters": {\n    "角色名": {\n      "role":"主角|反派|配角|主要角色","tag":"一句话评价",\n      "traits":[],"decision_style":"","risk_attitude":"",\n      "cares_about":[],"fears":[],"bottom_line":"",\n      "relationships":{},"trust_pattern":"","conflict_style":"",\n      "consistency_issues":[],"breaking_point":"","behavioral_model":""\n    }\n  },\n  "chapters": [{"number":1,"content":"完整原文"},{"number":2,"content":"完整原文"}],\n  "rhythm": {"per_chapter":[{"chapter":1,"page_turner":6,"emotion":4,"role":""}],"overall_pacing":"","top_questions":[],"next_chapter_hope":""},\n  "memo": {"writer_advice":[],"warnings":[]},\n  "skills": {"strengths":[{"aspect":"","examples":"","detail":""}],"weaknesses":[{"aspect":"","examples":"","detail":"","improvement":""}],"overall_advice":""}\n}');
    ...
    if(data.chapters&&data.chapters.length>0){
      p.chapters=data.chapters.map(ch=>({number:ch.number,content:ch.content,status:'draft',created_at:new Date().toISOString()}));
      p.chapters.forEach(ch=>{p.folders.chapters.push({file:'chapter_'+String(ch.number).padStart(3,'0')+'.md',content:ch.content});});
    } else if(hasRaw&&p.rawContent){ p.chapters=[{number:1,content:p.rawContent,status:'draft',created_at:new Date().toISOString()}]; p.folders.chapters.push({file:'chapter_001.md',content:p.rawContent}); }
    ...
    p.folders.fuzzyMemory['模糊记忆上下文.md']='# 模糊记忆上下文\n\n分析时间: '+new Date().toLocaleString()+'\n\n共 '+(data.chapters?.length||1)+' 章，已通过多轮对话完成分析。';
    saveDB();
    saveProjectFilesToDisk(name);
    _isAnalyzing = false;
    window.onbeforeunload = null;
    toast(`✅ 分析完成！${data.chapters?.length||chCount}章已提取剧情/角色/节奏数据`);
  } catch(e){
    _isAnalyzing = false;
    window.onbeforeunload = null;
    if(e.message!=='no key'){
...
function showChapterModal(chapterNum){
  ...
  const ch = (p.chapters||[]).find(c=>c.number===chapterNum);
  if(!ch){toast(`第${chapterNum}章内容不存在`,'error');return}
...
renderers.history = function(){
  ...
  const locked = getLockedProject();
  if(!locked){main.innerHTML=lockedMsg();side.innerHTML='';return}
  main.innerHTML = `<div class="page-title">📋 历史日志</div><div class="empty-state"><p>日志功能待实现</p></div>`;
  side.innerHTML = `<div class="card"><div class="card-title">📖 项目</div><div style="font-size:13px;color:var(--text-dim)">${locked.name}</div></div>`;
};
